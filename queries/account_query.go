...
const (
	...
	SelectAccountByAccountNumber GoQuery = `SELECT id, account_number, cif_number, status, created_at, updated_at, deleted_at FROM accounts WHERE account_number = ? AND deleted_at IS NULL`
	SelectAccountByID            GoQuery = `SELECT id, account_number, cif_number, status, created_at, updated_at, deleted_at FROM accounts WHERE id = ? AND deleted_at IS NULL`
